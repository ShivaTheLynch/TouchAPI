...
Func Core_Initialize($a_s_GW, $a_b_ChangeTitle = True)
	...
	; Ui
	Scanner_AddPattern('SendUIMessage', 'B900000000E8000000005DC3894508', -0x14, 'Func')
	...
	;Ui
	Memory_SetValue('SendUIMessage', Ptr(Scanner_GetScanResult('SendUIMessage', $g_ap_ScanResults, 'Func')))
    $l_p_Temp = Scanner_GetScanResult('CompassFlag', $g_ap_ScanResults, 'Func')
    Memory_SetValue('FlagHero', Ptr(Scanner_GetCallTargetAddress($l_p_Temp + 0x6E)))
	Memory_SetValue('FlagAll', Ptr(Scanner_GetCallTargetAddress($l_p_Temp + 0xA6)))
	...
	Memory_SetValue('LeaveGroup', Ptr(Scanner_GetCallTargetAddress($l_p_Temp + 0x176)))
	...
	;Ui log
	Log_Debug("SendUIMessage: " & Memory_GetValue('SendUIMessage'), "Initialize", $g_h_EditText)
