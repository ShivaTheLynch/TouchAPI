...
Func AggroMoveToEx($x, $y, $s = "", $z = 1700)
	...
	Do
		...
		If GetNumberOfFoesInRangeOfAgent(-2,1700,$GC_I_AGENT_TYPE_LIVING,1,"EnemyFilter") > 0 Then
			If GetPartyDead() Then ExitLoop
			If GetNumberOfMantisMendersInRangeOfAgent(-2, 1700) > 0 Then
				$enemy = GetNearestMantisMenderToAgent(-2, 1700)
			ElseIf GetNumberOfWardenSummersInRangeOfAgent(-2, 1700) > 0 Then
				$enemy = GetNearestWardenSummerToAgent(-2, 1700)
			ElseIf GetNumberOfWardenSeasonsInRangeOfAgent(-2, 1700) > 0 Then
				$enemy = GetNearestWardenSeasonToAgent(-2, 1700)
			Else
...
Func Fight($x, $s = "enemies")
	...
	Local $lastId = 99999, $coordinate[2], $timer
		Do
			...
			if GetPartyDead() Then ExitLoop
			If GetNumberOfMantisMendersInRangeOfAgent(-2, 1700) > 0 Then
				$target = GetNearestMantisMenderToAgent(-2, 1700)
			ElseIf GetNumberOfWardenSummersInRangeOfAgent(-2, 1700) > 0 Then
				$target = GetNearestWardenSummerToAgent(-2, 1700)
			ElseIf GetNumberOfWardenSeasonsInRangeOfAgent(-2, 1700) > 0 Then
				$target = GetNearestWardenSeasonToAgent(-2, 1700)
			Else
...
;~ Outpost - Map
Global Const $MAP_ID_DrazachThicket = 195
Global Const $Town_ID_EternalGrove = 222
Global Const $Town_ID_Great_Temple_of_Balthazar = 248
Global Const $Town_ID_EyeOfTheNorth = 642
Global Const $Town_ID_HouseZuHeltzer = 77
Global Const $Town_ID_Farm = $Town_ID_EternalGrove
