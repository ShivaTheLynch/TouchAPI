...
Func CreateModernGUI()
    $hGUI = GUICreate("GwAu3 Multi-Launcher", 1000, 720, -1, -1, -1, $WS_EX_WINDOWEDGE)
    GUISetBkColor($COLOR_BACKGROUND2, $hGUI)
    GUISetOnEvent($GUI_EVENT_CLOSE, "ExitApp")

    GUICtrlCreateLabel("", 0, 0, 1000, 60)
    ...
    CreateDualColorStatsCard(20, 80, "Selected / Total Accounts", "0", $COLOR_SUCCESS, "0", $COLOR_INFO, "lblSelectedCount", "lblTotalCount")
    ...
    _GUICtrlSmiley_Create($EMOJI_ONLINE, 950, 20, 20)

    $lblActivityText = GUICtrlCreateLabel("Ready", 860, 26, 80, 20, 0x0002)
    ...
    $btnClearAll = CreateEmojiButton(825, 510, 100, 32, "Clear All", $EMOJI_REMOVE, "ClearAllBots", $COLOR_ERROR)



    CreateGroupBox(20, 580, 950, 120, "Quick Actions")

    $btnMaster = CreateEmojiButton(40, 615, 120, 32, "Master Link", $EMOJI_NETWORK, "CreateMasterShortcut", $COLOR_ACCENT)
    $btnRefresh = CreateEmojiButton(170, 615, 120, 32, "Refresh", $EMOJI_REFRESH, "RefreshAccounts", $COLOR_INFO)
    $btnUpdate = CreateEmojiButton(300, 615, 120, 32, "Update Accounts", $EMOJI_SETTINGS, "Updater", $COLOR_SECONDARY)
    $btnHelp = CreateEmojiButton(430, 615, 120, 32, "Help", $EMOJI_HELP, "ShowHelp", $COLOR_SUCCESS)
    $btnShortcut = CreateEmojiButton(560, 615, 120, 32, "Shortcut", $EMOJI_SAVE, "CreateShortcut", $COLOR_SECONDARY)
    $btnActivityLog = CreateEmojiButton(690, 615, 120, 32, "Activity Log", $EMOJI_HELP, "ShowActivityLog", $COLOR_INFO)



    ; Create hidden log text control for logging functionality
    $g_h_LogText = _GUICtrlRichEdit_Create($hGUI, "", -1000, -1000, 1, 1, BitOR($ES_AUTOVSCROLL, $ES_MULTILINE, $WS_VSCROLL, $ES_READONLY))
    _GUICtrlRichEdit_SetBkColor($g_h_LogText, $COLOR_CARD_BG)
    _GUICtrlRichEdit_SetFont($g_h_LogText, 9, "Consolas")

    $progressBar = GUICtrlCreateProgress(40, 660, 920, 25)
...
Func ShowActivityLog()
    Local $hDialog = GUICreate("Activity Log", 600, 400, -1, -1, -1, BitOR($WS_EX_TOPMOST, $WS_EX_WINDOWEDGE), $hGUI)
    GUISetBkColor($COLOR_BACKGROUND2, $hDialog)
    GUISetOnEvent($GUI_EVENT_CLOSE, "CloseActivityLog")

    GUICtrlCreateLabel("", 0, 0, 600, 50)
    GUICtrlSetBkColor(-1, $COLOR_HEADER)

    GUICtrlCreateLabel("Activity Log", 20, 10, 300, 30)
    GUICtrlSetFont(-1, 14, 600, 0, "Segoe UI")
    GUICtrlSetColor(-1, $COLOR_HEADER_TEXT)
    GUICtrlSetBkColor(-1, $COLOR_HEADER)

    Local $hLogText = _GUICtrlRichEdit_Create($hDialog, "", 20, 70, 560, 280, BitOR($ES_AUTOVSCROLL, $ES_MULTILINE, $WS_VSCROLL, $ES_READONLY))
    _GUICtrlRichEdit_SetBkColor($hLogText, $COLOR_CARD_BG)
    _GUICtrlRichEdit_SetFont($hLogText, 9, "Consolas")

    ; Copy existing log content to the dialog
    If $g_h_LogText <> 0 Then
        Local $logContent = _GUICtrlRichEdit_GetText($g_h_LogText)
        _GUICtrlRichEdit_SetText($hLogText, $logContent)
    EndIf

    Local $btnClose = CreateEmojiButton(250, 360, 100, 32, "Close", $EMOJI_ERROR, "CloseActivityLog", $COLOR_ERROR)

    GUISetState(@SW_SHOW)
EndFunc

Func CloseActivityLog()
...
Func RefreshListView()
    ...
    For $i = 0 To UBound($aAccounts) - 1
        ...
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $launchSelection, 1)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $aAccounts[$i][0], 2)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $aAccounts[$i][2], 3)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $pid, 4)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $uptime, 5)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $keepAlive, 6)
        _GUICtrlListView_AddSubItem($lvAccounts, $item, $botName, 7)
    Next
...
Func ToggleKeepAlive($idx)
    ...
    If $aAccounts[$idx][5] = 0 Then
        $aAccounts[$idx][5] = 1
        _GUICtrlListView_SetItemText($lvAccounts, $idx, "☑", 6)
...
Func ToggleBotLaunchSelection($idx)
    ...
    If $aBotLaunchSelection[$idx] = 0 Then
        ...
        _GUICtrlListView_SetItemText($lvAccounts, $idx, "☑", 1)
...
Func SelectAllBots()
    ...
    For $i = 0 To UBound($aBotLaunchSelection) - 1
        ...
        _GUICtrlListView_SetItemText($lvAccounts, $i, $displayText, 1)
    Next
...
Func ClearAllBots()
    ...
    ; Update the ListView display for the Launch column
    For $i = 0 To UBound($aBotLaunchSelection) - 1
        _GUICtrlListView_SetItemText($lvAccounts, $i, "☐", 1)
    Next
